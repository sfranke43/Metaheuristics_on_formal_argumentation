...
int createNewSol(char*** currSol, char**** neighbors, char***** arr, int currSolLen, int* neighborsLen, int** neighborAtLen, int argcount,
                    int arrLen[argcount][3], double* realQualOfBest, int min, int max, double threshold,
...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < argc; i++)
    {
        ...
        if(strcmp(argv[i], "-n") == 0)
        {
            ...
            itCnt       = strtod(argv[i+6], NULL);
            decrease    = strtod(argv[i+7], NULL);
            coolChoice  = atoi(argv[i+8]);
        }
