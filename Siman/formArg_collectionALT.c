...
int createNewSol(char*** currSol, char**** neighbors, char***** arr, int currSolLen, int* neighborsLen, int** neighborAtLen, int argcount,
                    int arrLen[argcount][3], double* realQualOfBest, int min, int max, double threshold, char* problem, char* semantic, double punish, int* tabu, int tabuLen, int** actArr)
{
    printf("\n entered creatNewSol  ");
    ...
    *newSol = NULL;
    ...
    while(cnt < defLen && cnt < 10)
    {
        ...
        if(containsInt(tabu, -1 * (rand3pos + 1), tabuLen) == -1 )
        {
            ...
            //PROBLEM: BEI EINGABE VON KEINER INITIALSOL WIRD EINE LÖSUNG DER LÄNGE 0 ERZEUGT => FEHLER (SOLLTE GEFIXT SEIN)
    ...
    while(cnt < possibleAttLen && cnt < 0)
    {
        // choose and find the element
    ...
    while(cnt < currSolLen && cnt < 10)
    {
        ...
        if(containsInt(tabu, -1 * (rand2pos + 1), tabuLen) == -1 )
        {
            ...
            tabuLen++;
            oneNewSol(rand2pos, &curAct, currSol, newSol, arr, currSolLen, newSolLen, argcount,
                    arrLen, realQualOfBest, semantic, punish, tabu, tabuLen);

            //PROBLEM: BEI EINGABE VON KEINER INITIALSOL WIRD EINE LÖSUNG DER LÄNGE 0 ERZEUGT => FEHLER => GRUND?
    ...
    *neighborAtLen = resultAtLen;
    ...
    printf("neighborhood done len: %d", *neighborsLen);
...
* returns 0 if all arguments were tried and all solutions were not conflictfree
*/
int createNewSol1(char*** currSol, char**** neighbors, char***** arr, int currSolLen, int* neighborsLen, int** neighborAtLen, int argcount,
                    int arrLen[argcount][3], double* realQualOfBest, int min, int max, double threshold, char* problem, char* semantic, double punish, int* tabu, int tabuLen)
{
    printf("\n entered creatNewSol  ");
    ...
    *newSol = NULL;
    ...
    while(cnt + numberOfConlifctedSols < argcount && ((cnt < min || thresholdCnt < 10)&& cnt < max))
    {
        ...
        //choose a random argument that is not tabu and add tha chosen argument ro the tabu list so it is not tried again in this iteration
        int rand1 = myRandom(0, argcount-1, (tabu), tabuLen);
        if(containsInt(tabu, -1 * (rand1 + 1), tabuLen) != -1)
        {
            numberOfConlifctedSols++;
            continue;
        }
